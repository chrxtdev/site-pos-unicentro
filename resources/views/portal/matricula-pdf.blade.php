...
<body>
    ...
    <div class="content">
        <p>Declaramos, para os devidos fins, que o(a) estudante abaixo identificado(a) encontra-se regularmente
            matriculado(a) nesta instituição de ensino:</p>
        ...
        <div class="data-box">
            ...
            <div><strong>Status:</strong>
                {{ strtoupper($inscricao->status_pagamento == 'pago' ? 'Ativo' : 'Pendente') }}</div>
            <div><strong>Data de Inscrição:</strong> {{ $inscricao->created_at->format('d/m/Y') }}</div>
        </div>

        <p style="margin-top: 30px;">A referida pós-graduação é realizada na modalidade presencial/EAD, em conformidade
            com as diretrizes do Ministério da Educação (MEC).</p>
    </div>
