...
@section('content')
    ...
    <div class="mb-6 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
            ...
            <p class="text-slate-500 dark:text-slate-400 text-sm mt-1">Acompanhamento de matrículas, pagamentos e faturas dos alunos.</p>
        </div>
    </div>

    <!-- Filtros -->
    <div class="bg-white dark:bg-surface-dark rounded-xl shadow-sm border border-slate-200 dark:border-slate-700 p-5 mb-6">
        <form method="GET" action="{{ route('financeiro.index') }}" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
            
            <div class="md:col-span-2">
                <label for="search" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Buscar por Aluno</label>
                <div class="relative">
            ...
            <div>
                <label for="status_pagamento" class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Status Matrícula</label>
                <select name="status_pagamento" id="status_pagamento" class="block w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-600 rounded-lg focus:ring-primary focus:border-primary sm:text-sm text-slate-900 dark:text-white">
                    <option value="">Todos os Status</option>
                    <option value="pago" {{ request('status_pagamento') == 'pago' ? 'selected' : '' }}>Confirmado (Pago)</option>
                    <option value="pendente" {{ request('status_pagamento') == 'pendente' ? 'selected' : '' }}>Pendente</option>
                </select>
            </div>

            <div class="flex gap-2">
                <button type="submit" class="flex-1 bg-slate-800 dark:bg-primary text-white px-4 py-2 rounded-lg text-sm font-medium hover:bg-slate-700 dark:hover:bg-primary/90 transition relative top-[1px]">
                    <i class="fa-solid fa-filter mr-1"></i> Filtrar
                </button>
                @if(request()->anyFilled(['search', 'status_pagamento', 'curso']))
                    <a href="{{ route('financeiro.index') }}" class="px-4 py-2 bg-slate-100 dark:bg-slate-800 text-slate-600 dark:text-slate-400 rounded-lg text-sm font-medium hover:bg-slate-200 dark:hover:bg-slate-700 transition relative top-[1px] flex items-center justify-center" title="Limpar Filtros">
            ...
            <table class="min-w-full divide-y divide-slate-200 dark:divide-slate-700">
                <thead class="bg-slate-50 dark:bg-slate-800/50">
                    <tr>
                        ...
                        <th scope="col" class="px-6 py-4 text-center text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                            Data Matrícula
                        </th>
                        <th scope="col" class="px-6 py-4 text-center text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                            Mensalidade
                        ...
                        <th scope="col" class="px-6 py-4 text-right text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-wider">
                            Ações
                        </th>
                    </tr>
                </thead>
                <tbody class="bg-white dark:bg-surface-dark divide-y divide-slate-200 dark:divide-slate-700">
                    @forelse($alunos as $aluno)
                        <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors">
                                ...
                                <div class="flex items-center">
                                    ...
                                    <div class="ml-4">
                                        <div class="text-sm font-bold text-slate-900 dark:text-white flex items-center gap-2">
                                            {{ $aluno->nome }}
                                            @if($aluno->matricula)
                                                <span class="text-[10px] bg-slate-100 dark:bg-slate-800 px-1.5 py-0.5 rounded font-mono text-slate-500 border border-slate-200 dark:border-slate-700">RA: {{ $aluno->matricula }}</span>
                                            @endif
                                        </div>
                                        <div class="text-xs text-slate-500 dark:text-slate-400 max-w-xs truncate">{{ $aluno->pos_graduacao }}</div>
                                    </div>
                                </div>
                            </td>
                            ...
                            <td class="px-6 py-4 text-center whitespace-nowrap text-sm font-semibold text-slate-700 dark:text-slate-300">
                            ...
                            <td class="px-6 py-4 text-right whitespace-nowrap text-sm font-medium">
                                <a href="{{ route('financeiro.show', $aluno->id) }}" class="inline-flex items-center px-3 py-1.5 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-600 rounded-md text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-700 hover:text-primary dark:hover:text-primary focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-primary transition-colors shadow-sm">
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-10 text-center text-slate-500 dark:text-slate-400">
                                ...
                                <p class="text-lg font-medium text-slate-600 dark:text-slate-300">Nenhum aluno encontrado.</p>
