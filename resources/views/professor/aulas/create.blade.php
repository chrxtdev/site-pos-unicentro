...
@section('title', 'Registrar Aula - ' . $disciplina->nome)

@section('content')
<div class="max-w-4xl mx-auto flex flex-col gap-8 animate-fade-in">
    <div class="flex items-center justify-between px-4">
        <h3 class="text-xl font-bold text-slate-900 dark:text-white">Nova Aula e Chamada</h3>
    ...
    <form action="{{ route('professor.aulas.store', $disciplina) }}" method="POST" class="flex flex-col gap-8">
        @csrf
        ...
        <div class="glass-card p-8 rounded-[2rem] border border-slate-200 dark:border-white/5 shadow-soft">
            <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                <div>
                    ...
                    <input type="date" name="data" value="{{ date('Y-m-d') }}" required class="w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary transition-all">
                </div>
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-2">Hora (Opcional)</label>
                    <input type="time" name="hora" class="w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary transition-all">
                </div>
                <div>
                    <label class="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-2">Qtd de Aulas/Horas</label>
                    <input type="number" name="qtd_aulas" value="1" min="1" required class="w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary transition-all">
                </div>
            </div>
            <div class="mt-6">
                <label class="block text-[10px] font-black uppercase tracking-wider text-slate-400 mb-2">Conteúdo Ministrado</label>
                <textarea name="conteudo" rows="3" placeholder="Descreva brevemente o que foi tratado nesta aula..." class="w-full bg-slate-50 dark:bg-slate-900 border-slate-200 dark:border-slate-800 rounded-xl px-4 py-3 text-sm focus:ring-2 focus:ring-primary transition-all"></textarea>
            </div>
        </div>

        <!-- Chamada -->
        <div class="flex flex-col gap-4">
            <h3 class="text-lg font-bold text-slate-900 dark:text-white px-4">Chamada (Presença)</h3>
                            ...
                            <td class="p-4 text-center">
                                <div class="flex items-center justify-center gap-4">
                                    <label class="flex items-center gap-2 cursor-pointer group/radio">
                                        <input type="radio" name="chamada[{{ $matricula->signin_id }}]" value="1" checked class="w-4 h-4 text-emerald-500 border-slate-300 focus:ring-emerald-500">
                                    ...
                                    <label class="flex items-center gap-2 cursor-pointer group/radio-f">
                                        <input type="radio" name="chamada[{{ $matricula->signin_id }}]" value="0" class="w-4 h-4 text-red-500 border-slate-300 focus:ring-red-500">
        ...
        <button type="submit" class="w-full py-4 rounded-2xl bg-primary text-white font-black text-lg shadow-glow hover:scale-[1.02] active:scale-95 transition-all">
            Salvar Aula e Chamada
        </button>
    </form>
</div>
@endsection
